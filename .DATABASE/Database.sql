...
CREATE TABLE IF NOT EXISTS `administrator` (
  ...
  `username` varchar(32) CHARACTER SET latin1 NOT NULL DEFAULT '0',
  `password_hash` varchar(128) CHARACTER SET latin1 NOT NULL DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS `article` (
  ...
  KEY `fk_article_category_id` (`category_id`) USING BTREE,
  CONSTRAINT `fq_article_category_id` FOREIGN KEY (`category_id`) REFERENCES `category` (`category_id`) ON UPDATE CASCADE
...
CREATE TABLE IF NOT EXISTS `article_feature` (
  ...
  CONSTRAINT `fq_article_feature_article_id` FOREIGN KEY (`article_id`) REFERENCES `article` (`article_id`) ON UPDATE CASCADE,
  CONSTRAINT `fq_article_feature_feature_id` FOREIGN KEY (`feature_id`) REFERENCES `feature` (`feature_id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
CREATE TABLE IF NOT EXISTS `order` (
  ...
  `created_at` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `cart_id` int(10) unsigned NOT NULL DEFAULT '0',
  `status` enum('rejected',' accepted','shift','panding') DEFAULT 'panding',
...
CREATE TABLE IF NOT EXISTS `photo` (
  ...
  UNIQUE KEY `uq_phptp_image_path` (`image_path`),
  KEY `fk_photo_article_id` (`article_id`) USING BTREE,
  CONSTRAINT `fq_photo_article_id` FOREIGN KEY (`article_id`) REFERENCES `article` (`article_id`) ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
